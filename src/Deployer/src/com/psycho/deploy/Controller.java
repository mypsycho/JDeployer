...
public class Controller {
    ...
    public void startAll() {
        ...
        for (boolean lastFound=false; (lastToStart>=0) && (!lastFound); ) {
            ...
            lastFound = getWrapper(cmd).isConnected() && (!cmd.isRunning());
            if (!lastFound) {
                lastToStart--;
            }
        }
    ...
    public void startAll(ProcessWrapper process) {
       ...
       for (boolean lastFound=false; (lastToStart>=0) && (!lastFound); ) {
           ...
           lastFound = process.equals(getWrapper(cmd)) && (!cmd.isRunning());
           if (!lastFound) {
               lastToStart--;
           }
       }
       ...
       for (int iCmd=0; iCmd <= lastToStart; iCmd++) {
           ...
           if (process.equals(getWrapper(cmd))) {
               if (!cmd.isRunning()) {
                   try {
                       getWrapper(cmd).start(cmd);
                       oneStarted = true;
