...
import com.psycho.deploy.CommandTable.Column;
...
public class ControlPane extends JSplitPane implements ControlListener {
    ...
    protected CommandTable commands = new CommandTable(this);
    protected CommandHeader cmdEditor = new CommandHeader(this);
    protected JTable cmdTable = new JTable(commands);
    
    protected JTabbedPane switchPane = new JTabbedPane();
    
    protected MoveableTable mainTable = new MoveableTable(this);
    ...
    public ControlPane(Controller pCore) {
        ...
        newWrapper = new WrapperHeader(this) {
            protected void update() {
                ...
                startMoreB.setVisible(false);
                stopMoreB.setVisible(false);
            }
        };
        ...
        h.with(RIGHT, switchPane)
            ...
            .with("process", new BorderLayout())
                .add("id", newWrapper, BorderLayout.PAGE_START)
        ...
        cmdTable.getColumnModel().getColumn(Column.name.ordinal()).setPreferredWidth(100);
        cmdTable.getColumnModel().getColumn(Column.cmd.ordinal()).setPreferredWidth(300);
        cmdTable.getColumnModel().getColumn(Column.state.ordinal()).setPreferredWidth(40);

        ((JTabbedPane) h.get(RIGHT)).setEnabledAt(1, list.getSelectedValue() != null);
...
//        SwingUtil.setJPanelOpaque(processDisplay, false);
//        SwingUtil.setJPanelOpaque(sequenceDisplay, false);
    ...
    protected class ListEventListenener extends MouseAdapter implements ListSelectionListener {
        ...
        public void tableClicked(MouseEvent me) {
            if (me.getClickCount() == 1) {
                ...
                try {
                    Command toToggle = selected.get(rowLaunch);
                    if (!selected.isConnected()) {
                        selected.connect();
                        wrapper.update();
                    }
                    if (!toToggle.isRunning()) {
                        selected.start(toToggle);
                    } else {
                        selected.stop(toToggle);
                    }
                } catch (Exception e) {
    ...
    public void startCommands(ProcessWrapper pProcess) throws IllegalStateException {
        if (launching != null) { // Inner error => No translation
            throw new IllegalStateException("Sequence already in progress");
        }
        launching = new Launcher(pProcess); // inner class
